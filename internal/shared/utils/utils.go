...
// RemoveFinalizers removes finalizers from all instances of the specified custom resource type
func RemoveFinalizers(ctx context.Context, logger logr.Logger, c client.Client, resourceType interface{}) error {
	...
	for i := 0; i < items.Len(); i++ {
		...
		if ContainsString(item.GetFinalizers(), vars.FINALIZER) {
			item.SetFinalizers(RemoveString(item.GetFinalizers(), vars.FINALIZER))
			if err := c.Update(ctx, item.(client.Object)); err != nil {
				logger.Error(err, "unable to remove finalizer", "name", item.GetName(), "namespace", item.GetNamespace())
				return err
			}
			logger.Info("Removed finalizer", "name", item.GetName(), "namespace", item.GetNamespace())
		}
	}
