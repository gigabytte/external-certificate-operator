...
func NewOpenSSLRunners(ctx context.Context) *OpenSSLRunners {
...
func (osr *OpenSSLRunners) ProcessCertificates() error {
	log := log.FromContext(osr.Ctx)

	// Process TLSCrt
	if len(osr.Certificates.TLSCrt) > 0 {
		log.Info("Processing TLSCrt")
		if err := osr.GetCertificateExpiration(osr.Certificates.TLSCrt); err != nil {
			log.Error(err, "Failed to get certificate expiration for TLSCrt")
			return err
		}
	} else {
		log.Info("TLSCrt is empty, skipping")
	}

	// Process CACrt
	if len(osr.Certificates.CACrt) > 0 {
		log.Info("Processing CACrt")
		if err := osr.GetCertificateExpiration(osr.Certificates.CACrt); err != nil {
			log.Error(err, "Failed to get certificate expiration for CACrt")
			return err
		}
	} else {
		log.Info("CACrt is empty, skipping")
	}

	// Process PemBundle
	if len(osr.Certificates.PemBundle) > 0 {
		log.Info("Processing PemBundle")
		if err := osr.DecodePEM(); err != nil {
			log.Error(err, "Failed to decode PEM for PemBundle")
			return err
		}
		if err := osr.CreatePKCS12(); err != nil {
			log.Error(err, "Failed to create PKCS12 for PemBundle")
			return err
		}
		if len(osr.Certificates.ConvertedFormats.PFXCertificate) > 0 {
			if osr.Certificates.CertificateDetails.ExpirationTime == 0 {
				if err := osr.GetCertificateExpiration(osr.Certificates.ConvertedFormats.PFXCertificate); err != nil {
					log.Error(err, "Failed to get certificate expiration for PemBundle")
					return err
				}
			}
		}
		...
		log.Info("PemBundle is empty, skipping")
	}

	log.Info("All certificate types processed successfully")
	return nil
}
...
func (osr *OpenSSLRunners) CreatePKCS12() error {
	...
	for _, cert := range certs {
		if time.Now().Before(cert.NotBefore) || time.Now().After(cert.NotAfter) {
			log.Error(nil, "Certificate is either not yet valid or has expired")
	...
	if err != nil {
		log.Error(err, "Failed to create temp file for key")
		return fmt.Errorf("failed to create temp file for key: %w", err)
	}
	defer func() {
		if cerr := keyFile.Close(); cerr != nil {
			log.Error(cerr, "Failed to close key temp file for cert")
		}
		if cerr := os.Remove(keyFile.Name()); cerr != nil {
			log.Error(cerr, "Failed to remove key temp file for cert")
		}
	}()

	certFile, err := os.CreateTemp("", "cert-*.pem")
	if err != nil {
		log.Error(err, "Failed to create temp file for cert")
		return fmt.Errorf("failed to create temp file for cert: %w", err)
	}
	defer func() {
		if cerr := certFile.Close(); cerr != nil {
			log.Error(cerr, "Failed to close temp file for cert")
		}
		if cerr := os.Remove(certFile.Name()); cerr != nil {
			log.Error(cerr, "Failed to remove temp file for cert")
		}
	}()

	privateKey, ok := key.(*rsa.PrivateKey)
	if !ok {
		log.Error(nil, "Key is not of type *rsa.PrivateKey")
	...
	if err := pem.Encode(keyFile, &pem.Block{Type: "RSA PRIVATE KEY", Bytes: x509.MarshalPKCS1PrivateKey(privateKey)}); err != nil {
		log.Error(err, "Failed to write key to PEM")
	...
	for _, cert := range certs {
		if err := pem.Encode(certFile, &pem.Block{Type: "CERTIFICATE", Bytes: cert.Raw}); err != nil {
			log.Error(err, "Failed to write cert to PEM")
	...
	if err := keyFile.Sync(); err != nil {
		log.Error(err, "Failed to sync key file")
		return fmt.Errorf("failed to sync key file: %w", err)
	}
	if err := certFile.Sync(); err != nil {
		log.Error(err, "Failed to sync cert file")
...
func (osr *OpenSSLRunners) DecodePEM() error {
	...
	for {
		...
		if block == nil {
			log.Error(nil, "Failed to parse PEM block")
	...
	if len(certs) == 0 || key == nil {
		log.Error(nil, "Certificate or key missing in PEM")
	...
	log.Info("PEM decoded successfully")
...
func (osr *OpenSSLRunners) GetCertificateExpiration(certByte []byte) error {
	...
	// Check if certByte contains a certificate
	if !strings.Contains(string(certByte), "-----BEGIN CERTIFICATE-----") {
		log.Error(nil, "certByte does not contain a valid certificate")
		return fmt.Errorf("certByte does not contain a valid certificate")
	}
	...
	defer func() {
		if cerr := os.Remove(tmpfile.Name()); cerr != nil {
			log.Error(cerr, "Failed to remove temp file")
		}
	}()

	// Write the certificate string to the temporary file
	if _, err := tmpfile.Write(certByte); err != nil {
		log.Error(err, "Failed to write certificate to temporary file")
		return err
	}
	if err := tmpfile.Close(); err != nil {
		log.Error(err, "Failed to close temporary file")
		return err
	}
	log.Info("Certificate written to temporary file", "fileName", tmpfile.Name())
	...
	err = cmd.Run()
	if err != nil {
		log.Error(err, "Failed to run OpenSSL command")
		return err
	}

	output := out.String()
	log.Info("OpenSSL command output", "output", output)

	// Example output: "notAfter=Dec 31 23:59:59 2024 GMT"
	parts := strings.Split(output, "=")
	if len(parts) != 2 {
		log.Error(nil, "Unexpected OpenSSL output", "output", output)
		return fmt.Errorf("unexpected output: %s", output)
	}

	dateStr := strings.TrimSpace(parts[1])
	log.Info("Parsed date string from OpenSSL output", "dateStr", dateStr)

	expirationDate, err := time.Parse("Jan 2 15:04:05 2006 MST", dateStr)
	if err != nil {
		log.Error(err, "Failed to parse expiration date")
		return err
	}

	osr.Certificates.CertificateDetails.ExpirationTime = float64(expirationDate.Unix())
	log.Info("Certificate expiration date retrieved successfully", "expirationTime", osr.Certificates.CertificateDetails.ExpirationTime)
