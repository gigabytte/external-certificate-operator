...
func (aza *WorkloadID) AuthorizerForWorkloadIdentity() error {
	...
	if !ok {
		log.Error(fmt.Errorf("client ID annotation not found"), "ServiceAccount does not have the required client ID annotation")
	...
	if !ok {
		log.Error(fmt.Errorf("tenant ID annotation not found"), "ServiceAccount does not have the required tenant ID annotation")
